...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return operator.eval_call(expr.second, env)
        # END PROBLEM 5
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('Arguments and parameters length mismatch')
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        return scheme_apply(self, operands.map(lambda self: scheme_eval(self, env)), env)
        # END PROBLEM 5
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('unexpected number of arguments to {0}'.format(self.fn))
        # END PROBLEM 4
...
def do_define_macro(expressions, env):
    ...
    target = expressions.first
    if scheme_symbolp(target):
        check_form(expressions.first, 2, 2)
        env.define(expressions.first, MacroProcedure(expressions.first.second, expressions.second), env)
        return expressions.first
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        name = target.first
        env.define(name, MacroProcedure(expressions.first.second, expressions.second, env))
        return name
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
