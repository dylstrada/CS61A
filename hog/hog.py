...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for _ in range(num_rolls):
        #temporary value roll_score so that dice() is called once per loop
        roll_score = dice()
        #if player rolls a one, "Pig Out" rule takes effect
        if roll_score == 1:
            pig_out = True
        points += roll_score
    if pig_out:
...
def free_bacon(score):
    ...
    # BEGIN PROBLEM 2
    # Compare 10s place to 1s place, then add 1 to the max
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    # BEGIN PROBLEM 7
    def say(score0, score1):
        # Sets current high to previous high
        current_high = previous_high
        if (who == 0):
            # If new gain is bigger than previous high
            if (score0 - previous_score > previous_high):
                # Set current high to new gain
                current_high = score0 - previous_score
                if (current_high == 1):
                    print(current_high, "point! That's the biggest gain yet for Player", who)
                else:
                    print(current_high, "points! That's the biggest gain yet for Player", who)
            # Return function with the updated arguments
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    # Stores the highest average scored by rolling dice i times
    max_average = 0
    # Target number of rolls
    target = 1
    for i in range(1, 11):
        # Current average scored by rolling dice i times
        test = make_averaged(roll_dice, num_samples)(i, dice)
        # If current average is greater than the previous
        if (test > max_average):
            max_average = test
            target = i
    return target
...
def bacon_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 10
    # Check if free bacon satisfies the parameters
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 11
    # Check if hypothetical free bacon score can swap
    if (is_swap(score + free_bacon(opponent_score), opponent_score)):
        # Check if swapping is beneficial
        if score + free_bacon(opponent_score) < opponent_score:
            return 0
    # Check if bacon_strategy suggests rolling a 0
