...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 11
                if Ant.can_contain(self.ant, insect):
                    self.ant.ant = insect
                elif Ant.can_contain(insect, self.ant):
                    temp = self.ant
                    self.ant = insect
                    insect.ant = temp
                else:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if type(self.ant) is QueenAnt:
                if self.ant.is_impostor:
                    self.ant = None
                ...
                if self.ant.container and type(self.ant.ant) is not QueenAnt:
                    self.ant.ant = None
                elif self.ant.container and type(self.ant.ant) is QueenAnt:
                    if self.ant.ant.is_impostor:
                        self.ant.ant = None
                else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        if self.place.bees:
            ...
            transitions = 1
            while next_place is not hive:
                if next_place.bees:
                    if self.min_range <= transitions <= self.max_range:
                        return random_or_none(next_place.bees)
                if transitions > self.max_range:
                    break
                else:
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_impostor:
            ...
            while next_place:
                ant = next_place.ant
                if ant and not ant.is_doubled and hasattr(ant, 'damage') and type(ant) is not QueenAnt:
                    ant.damage *= 2
                    ant.is_doubled = True
                if ant and ant.container and ant.ant and not ant.ant.is_doubled and hasattr(ant.ant, 'damage') and type(ant.ant) is not QueenAnt:
                    ant.ant.damage *= 2
                    ant.ant.is_doubled = True
